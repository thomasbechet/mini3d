...
use mini3d::renderer::server::{TextureHandle, ViewportHandle};
use wgpu::{include_wgsl, vertex_attr_array};

use crate::{
    context::WGPUContext,
    graphics_canvas::{GraphicsCanvas, CANVAS_COLOR_FORMAT, CANVAS_DEPTH_FORMAT},
    graphics_render_pass::GraphicsCommand,
    texture::Texture,
    viewport::Viewport,
};
...
impl GraphicsPipelines {
    fn new(
        ...
        let primitive_triangles_pipeline =
            context
                .device
                .create_render_pipeline(&wgpu::RenderPipelineDescriptor {
...
impl GraphicsRenderer {
    pub(crate) fn new(context: &WGPUContext) -> Self {
        ...
        let blit_shader = context
            .device
            .create_shader_module(include_wgsl!("shader/graphics_blit.wgsl"));
        let primitive_shader = context
            .device
            .create_shader_module(include_wgsl!("shader/graphics_primitive.wgsl"));
                    ...
                    entries: &[wgpu::BindGroupLayoutEntry {
                        ...
                        ty: wgpu::BindingType::Buffer {
                            ...
                            min_binding_size: wgpu::BufferSize::new(
                                std::mem::size_of::<GPUGlobalData>() as u64,
                            ),
                        },
                        count: None,
                    }],
            ...
            match command {
                ...
                GraphicsCommand::Viewport(cmd) => {
                    if let hash_map::Entry::Vacant(e) =
                        self.viewport_bind_groups.entry(cmd.viewport)
                    {
        ...
        for command in canvas.render_pass.commands.iter() {
            match command {
                GraphicsCommand::Blit(cmd) => {
                    ...
                    let start =
                        cmd.instance_start as u64 * std::mem::size_of::<GPUBlitData>() as u64;
                    let stop = start
                        + cmd.instance_count as u64 * std::mem::size_of::<GPUBlitData>() as u64;
                    render_pass.set_bind_group(
                        1,
                        self.texture_bind_groups.get(&cmd.texture).unwrap(),
                        &[],
                    );
                    render_pass
                        .set_vertex_buffer(0, canvas.render_pass.blit_buffer.slice(start..stop));
                ...
                GraphicsCommand::Viewport(cmd) => {
                    ...
                    render_pass.set_bind_group(
                        1,
                        self.viewport_bind_groups.get(&cmd.viewport).unwrap(),
                        &[],
                    );
                    render_pass
                        .set_vertex_buffer(0, canvas.render_pass.blit_buffer.slice(start..stop));
                ...
                GraphicsCommand::Triangles(cmd) => {
                    ...
                    render_pass.draw(
                        cmd.vertex_start..(cmd.vertex_start + cmd.vertex_count),
                        0..1,
                    );
                }

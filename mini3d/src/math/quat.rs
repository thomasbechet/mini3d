...
impl<T: FixedPoint + SignedFixedPoint + TrigFixedPoint> Q<T> {
    ...
    pub fn mul_vec3(self, v: V3<T>) -> V3<T> {
        ...
        u * u.dot(v) * 2 + v * (self.w * self.w - u.dot(u)) + u.cross(v) * self.w * 2
    }
    ...
    pub fn to_mat4(mut self) -> M4<T> {
        ...
        M4::from_rows_element(
            (
                T::ONE - self.y * self.y * 2 - self.z * self.z * 2,
                self.x * self.y * 2 - self.z * self.w * 2,
                self.x * self.z * 2 + self.y * self.w * 2,
                T::ZERO,
            ),
            (
                self.x * self.y * 2 + self.z * self.w * 2,
                T::ONE - self.x * self.x * 2 - self.z * self.z * 2,
                self.y * self.z * 2 - self.x * self.w * 2,
                T::ZERO,
            ),
            (
                self.x * self.z * 2 - self.y * self.w * 2,
                self.y * self.z * 2 + self.x * self.w * 2,
                T::ONE - self.x * self.x * 2 - self.y * self.y * 2,
                T::ZERO,
            ),
...
impl<T: FixedPoint + SignedFixedPoint + TrigFixedPoint> Mul for Q<T> {
    type Output = Self;
...
impl<T: FixedPoint + SignedFixedPoint + TrigFixedPoint> MulAssign for Q<T> {
